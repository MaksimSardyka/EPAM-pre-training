...
public class IterativeLogic {
	...
	public static String hanoi(int numberRing, char sourceTower, char targetTower, char temporaryTower) {
		...
		for (int i = 0; i < limit; i++) {
			...
			int source = (calcMovements(i, currentDisk) * getDirection(currentDisk, numberRing)) % towers.length; // find
																													// source
																													// tower
			int target = (source + getDirection(currentDisk, numberRing)) % towers.length; // find target tower
	...
	private static int getDirection(int currentDisk, int numberRing) { // direction for this disk: clockwise=1,
																		// anticlockwise=2
